...
## Schritt 0.5: Meta-Block lesen (Typ-spezifische Konfiguration)

Bevor du reviewst, lese den **Meta-Block** aus `04-fortschritt/Fortschritt.md`. Dieser Block enthält strukturelle Metadaten, die dein Review konfigurieren:

```yaml
# Meta-Block (Anfang der Fortschritt.md)
arbeitstyp: Bachelorarbeit | Hausarbeit | Seminararbeit | Masterarbeit
seitenzahl_ziel: [integer] # z.B. 50 für BA
zitierstil: Harvard | Deutsche Zitierweise | Chicago
empirie: true | false
fachbereich: [string] # z.B. BWL
```

**Was der Meta-Block dir sagt:**
- **arbeitstyp** → Bestimmt die Anforderungsschwelle. Hausarbeiten (15 pp) haben niedrigere Anforderungen als Masterarbeiten (60–80 pp).
- **seitenzahl_ziel** → Informiert deine Bewertung. Ein Kapitel in einer 50-seitigen BA sollte mehr Substanz haben als eines in einer 15-seitigen Hausarbeit.
- **zitierstil** → Prüfe nur den tatsächlich verwendeten Stil (Harvard mit Obsidian-Links ODER Deutsche Zitierweise mit Markdown-Fußnoten).
- **empirie** → Falls `true`: Prüfe nicht nur Literaturarbeit, sondern auch Qualität der Datenauswertung (Kodierung, Statistik, Triangulation).
- **fachbereich** → Kontext für Fachsprachlichkeit und typische Argumentationsmuster.

Lese auch die **Foundation-Dateien**, auf die der Meta-Block verweist:
- `_foundation/arbeitstypen.md` — Kapiteltypen und Seitenzahl-Budgets pro Arbeitstyp
- `_foundation/validierung.md` — Review-Kriterien und Priorisierung pro Arbeitstyp (Hausarbeit: Kompaktheit vor Umfang; BA: Tiefe + Breite; MA: Kritische Reflexion)
- `_foundation/zitierstile.md` — Formale Anforderungen des gewählten Zitierstils
- `_foundation/wissenschaftlicher-stil.md` — Fachsprachliche Konventionen

**Praktisch:** Wenn du einen Aufsatz in einer Hausarbeit reviewst, priorisierst du anders als einen theoretischen Rahmen in einer Masterarbeit. Der Meta-Block sagt dir, wie streng du sein sollst.
...
- Überarbeitung nötig → Weiter mit `studienarbeit-ueberarbeitung`; Muss-Punkte adressieren
- Grundlegende Überarbeitung → Weiter mit `studienarbeit-ueberarbeitung`; ggf. Teile neu schreiben
...
**Für Literatur-Strang:**
- Quellendichte und -häufigkeit relativ zu Arbeitstyp und Seitenzahl (vgl. `arbeitstypen.md`)
- Quellenqualität und -aktualität (peer-reviewed, grey literature, Grenzliteratur korrekt deklariert)
- Quellenvielfalt (Monographien, Zeitschriftenartikel, Graue Literatur)
- Zitierformat-Konsistenz (Harvard/Deutsche Zitierweise/Chicago gemäß Meta-Block) – vgl. `zitierstile.md`
- Abgleich: Quellenauswertung ↔ Kapitel (alle Zitate kommen aus Auswertung?)
- `[QUELLE ERGÄNZEN]`-Stellen – sind diese substantiell oder nur Lücken im Schreibprozess?
- Quellentabelle: vollständig und mit Meta-Informationen?

**Für Empirik-Strang** (falls `empirie: true` in Meta-Block):
- Datenauswertungsmethodik: Kodierung (Mayring, Kuckartz) oder Statistik (deskriptiv/inferenziell) dokumentiert und korrekt angewendet
- Kodier-/Analyse-Integrität: Intercoder-Übereinstimmung / Reliabilität bei Statistik
- Statistische Korrektheit: Test-Auswahl, Voraussetzungen, p-Werte, Effektstärken adäquat?
- Datenquellen-Transparenz: Erhebungsmethode, Stichprobe, Ausfallquoten, Einschlusskriterien klar?
- Triangulation: Wenn Multiple Methoden, sind diese sinnvoll integriert?
- Quellenintegration: Werden Daten und Literatur sinnvoll miteinander verknüpft?
...
Die vier Bewertungsstufen und ihre Bedeutung (vgl. `_foundation/validierung.md` für arbeitstyp-spezifische Konkretisierungen):
- **Stark**: Erfüllt die Anforderungen einer sehr guten Abschlussarbeit für die Arbeitstyp und Seitenzahl → Freigegeben
...
- **Schwach**: Grundlegende Mängel, erheblich unterhalb der Anforderungen für den Arbeitstyp → Grundlegende Überarbeitung nötig
